...
8. **前端两个视图**——原始血缘图（详细表/字段关系，已存在）+ 语义血缘图（业务分组 → 目标中间表 → 下游消费）。**当前进度** ✅：`SemanticLineagePanel.vue` 显示 observations / risks / 业务分组卡片 + 跨分组依赖（rule_order 着色 7 色循环）/ 目标表（含 role + refresh + titles + counts）/ procedures。把 a_cispnew_f3045.sql 配 example yaml 跑出来：7 个分组（集中交易 / 融资融券 / 期权 / 机构客户 / 持仓市值 / 配置 / 过滤排除），跨分组依赖 7 条（融资融券→集中交易 ×78、机构客户→集中交易 ×81、机构客户→持仓市值 ×26 等）。完整的"业务分组级 DAG 视图"还没做（依赖 G6 重写一版精简图），但卡片视图已经能让用户一眼看清"哪些业务流向哪些业务"。
