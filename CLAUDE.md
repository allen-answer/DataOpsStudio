...
- **TypeScript 渐进迁移**（S3.B 全 10 store + S4.A api.ts + composable + S4.B codegen 落地，2026-05-10 启动 view 迁移 batch 1）：10/10 store + api.ts（含泛型 apiGet&lt;T&gt; / apiJson&lt;T&gt;）+ useLineageGraphData composable 全部 ts；openapi-typescript 从 /openapi.json 自动生成 `src/types/api-schema.ts`，友好别名在 `src/types/api.ts`（auth / project / datasource / task / workflow 已用）。view 迁移 batch 1（5 个最简 thin shell）：BatchView / DatasourceView / WorkbenchView / WorkflowView / LineageView 全转 `<script setup lang="ts">`，加 StepId / SubPageId 等关键 union 类型 + `route.params.x as string | undefined`。batch 2（4 个中等 view）：HistoryView（纯 store-driven）/ LoginView（typed onSubmit）/ WorkflowTemplateView（TemplateItem interface）/ AuditLogView（AuditLogEntry interface + apiGet 泛型）。batch 3（4 个 view）：LineageReportView（TabId union）/ WorkflowListView（owner/health filter typed）/ UserManagementView（Role union + UserItem）/ ProjectManagementView（UserItem / ProjectItem interface + memberDraft 泛型）。batch 4（4 个 view）：SchedulerMonitorView（SchedulerStatus / TickResult interface + setInterval ReturnType）/ LineageWorkbenchView（ModeId / Pipeline union + onSingleFileChange 等 file event handler 类型化）/ WorkflowRunView（reusableVars / parseTs / stepBarStyle 类型化）/ AIConfigView（AIConfigPayload / AITestResult interface + apiGet/apiJson 泛型）。仍剩 ~3 个 view 待迁（AspectGovernanceView / WorkflowDetailView / AssetDetailView 906 行大头）下个 batch 接力
