...
def test_streaming_excel_max_rows_caps_summary(tmp_path):
    ...
    wb = load_workbook(out_path)
    # diff 先吃 2 行就到 max，only_source 一行都不写
...
def test_build_excel_for_parquet_uses_streaming_path(isolated_storage):
    """build_excel_for_run 对 parquet runs 走 write_excel_streaming —— 验证
    monkey-patch 显示 streaming function 被调用、老 _load_buckets_from_parquet
    没被走。"""
    import importlib
    ...
    # Excel 文件落到 <run_id>/export.xlsx，可读
    out = isolated_storage["results"] / "RUN_F4" / "export.xlsx"
    assert out.exists()
    wb = load_workbook(out)
    assert wb["only_source"].max_row == 1 + 10   # max_rows 全分到 diff... 但 diff 空，only_source 拿满 10
...
def test_collect_bucket_columns_from_meta_extracts_struct_fields(isolated_storage):
    ...
    # pyarrow struct 字段顺序非 dict 插入序（首批写入时锁的），所以用 set 断言
    assert set(cols["only_source"]["source"]) == {"id", "name", "amt"}
    assert cols["only_source"]["target"] == []   # only_source 无 target struct
    assert cols["diff"] == {"source": [], "target": []}  # 空桶兜底


def test_streaming_excel_keeps_max_rows_cap_for_parquet_run(isolated_storage):
    """parquet run 走 F.4 streaming 后 max_rows 仍生效（覆盖 P1 patch 的回归）。"""
    task_id = _create_task()
    big = {
