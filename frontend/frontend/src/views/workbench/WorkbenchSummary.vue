...
// "更多操作" 折叠区：后台执行 / 复制 / 删除 都收在这里，默认收起。
...
<template>
  <aside class="card sticky top-4 flex max-h-[calc(100vh-2rem)] flex-col overflow-hidden">
    <!-- 摘要内容区：可滚动，操作按钮固定在底部不随它滚 -->
    ...
    <!-- 操作区：固定在底部，不随摘要滚动。只保留一个主按钮「执行对比」 -->
    <div class="shrink-0 space-y-1.5 border-t border-slate-100 pt-3">
      <button
      ...
      >
        <MoreHorizontal class="h-3.5 w-3.5" /> 更多操作
      ...
      <div v-if="showMore" class="space-y-1.5">
        ...
        <button class="btn btn-outline h-9 w-full text-xs" :disabled="!isSavedTask" @click="copyTask">
          <Copy class="h-3.5 w-3.5" /> 复制任务
        </button>
        <button class="btn btn-danger h-9 w-full text-xs" :disabled="!isSavedTask" @click="deleteTask">
