...
def fetch_schema_metadata(
    ...
    sql = _schema_query(source, schema_name=schema_name, table_filter=table_filter, include_tables=include_tables, schema_dialect=schema_dialect)
...
def _schema_dialect(source: SchemaSource, override: str = "") -> str:
    value = (override or source.extra.get("schema_dialect") or source.extra.get("schema_mode") or str(getattr(source.db_type, "value", source.db_type))).strip().lower()
    aliases = {
        "mysql": "mysql",
        "oracle": "oracle",
        "dm": "dm",
