...
def write_excel_streaming(
    ...
    *,
    ...
    # 汇总 sheet 列集：source 并集 + target 并集 + 是否存在 + 差异字段
    ...
    # 4 个 per-bucket sheet 提前 create + 写 header；保存 layout 给行写入用
    per_bucket_ws: dict[str, tuple[Any, dict[str, list[str]]]] = {}
    for name in _SHEET_ORDER:
        ws = wb.create_sheet(name)
        layout = _write_streaming_bucket_header(ws, name, bucket_columns.get(name) or {})
        per_bucket_ws[name] = (ws, layout)

    remaining = max_rows if max_rows is not None else None
    for bucket_name in _SHEET_ORDER:
        if remaining is not None and remaining <= 0:
            break
        ws_per, per_layout = per_bucket_ws[bucket_name]
        for row in bucket_iter_factory(bucket_name):
            ...
            _append_bucket_row(ws_per, per_layout, row)
...
def _union_columns(
    ...
    *,
    ...
    """按 _SHEET_ORDER 出现顺序保留 + 去重 —— 跟老 _side_columns 同语义。"""
    seen: set[str] = set()
    out: list[str] = []
...
def _write_streaming_bucket_header(
    ...
    """每个 per-bucket sheet 的 header：key + source.<col> + target.<col> + changes（仅 diff）。
    返回 layout 给 _append_bucket_row 用。"""
    header_font = Font(bold=True)
    fill = PatternFill("solid", fgColor=_BUCKET_FILL_COLORS.get(bucket_name, "FFFFFF"))
...
def _append_summary_row(
    ...
    """汇总 sheet 一行：源列值 + 目标列值 + 是否存在 + 差异字段。带 bucket 颜色 fill。"""
    fill = PatternFill("solid", fgColor=_BUCKET_FILL_COLORS.get(bucket_name, "FFFFFF"))
...
def _append_bucket_row(
    ...
    """per-bucket sheet 一行：key + source.<col> + target.<col> + (changes)。"""
    source = row.get("source") or {}
    target = row.get("target") or {}
    cells: list[Any] = [
